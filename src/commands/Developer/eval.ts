...
export default class PingCommand extends Command {
    ...
    async run(message: Message, args: string[]): Promise<void | Message> {
        if (!args[0]) return message.channel.send('What do you evaluate?');
        switch (args[0].toLowerCase()) {
            case '-a': {
                if (!args[1]) return message.channel.send('What do you evaluate?');
                try {
            ...
            case '-sh': {
                if (!args[1]) return message.channel.send('What should I put in the terminal?');
                let evalued = args.slice(1).join(' ')
                try {
                    const { stdout, stderr } = await exec(evalued);
                    if (!stdout && !stderr) return message.channel.send('I executed that but there\'s no nothing to show.');
                    if (stdout)
